...
const clickOptions = () => {
  ...
  const $containers = $(
    "#NM_NEWSSTAND_DEFAULT_LIST ._NM_NEWSSTAND_ARTICLE_CONTAINER"
  );
  ...
  on: {
    slideChangeTransitionStart: function (swiper) {
      ...
      if (index === 0) {
        ...
        $(".option_item_7").trigger(
          "click",
          totalSwiper.slides.length === 1 ? 1 : totalSwiper.slides.length - 2
        );
      } else if (index > total) {
...
const clickSortArea = () => {
  ...
  $sortBtns.on("click", (e) => {
    ...
    if ($self.data("type") === "my") {
      ...
      $('#NM_NEWSSTAND_data_buttons .btn_sort[data-type="my"]').addClass(
        "sort_on"
      );
    } else {
      ...
      $('#NM_NEWSSTAND_data_buttons .btn_sort[data-type="all"]').addClass(
        "sort_on"
      );
    }
  });
